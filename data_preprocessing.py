...
# df_drugs_smiles = pd.read_csv('data/twosides_smiles.csv')
...
#         stack = []
#         pred = torch.full((num_nodes,), -1, dtype=torch.long)
...
#                         pred[w] = v
#
#         delta = torch.zeros(num_nodes)
#         while stack:
#             w = stack.pop()
#             coeff = (1 + delta[w]) / sigma[w]
#             for v in range(num_nodes):
#                 if adjacency_matrix[w][v] != 0:
...
def get_bipartite_graph(mol_graph_1, mol_graph_2):
    ...
    # num_nodes1 = len(mol_graph_1.GetAtoms())
    # num_nodes2 = len(mol_graph_2.GetAtoms())
    # edge_list_1 = torch.LongTensor([(b.GetBeginAtomIdx(), b.GetEndAtomIdx()) for b in mol_graph_1.GetBonds()])
    # adjacency_matrix = torch.zeros(num_nodes1, num_nodes1)
    # for edge in edge_list_1:
    #     adjacency_matrix[edge[0], edge[1]] = 1
    #     adjacency_matrix[edge[1], edge[0]] = 1
    # node_importance = betweenness_centrality(adjacency_matrix)
    # # 设置重要性阈值
    # threshold = 0.1
    # # 根据阈值删除不重要的节点
    # nodes_to_remove = [node for node, importance in enumerate(node_importance) if importance < threshold]
    # x1 =[x for x in x1 if x not in nodes_to_remove]
    #
    # edge_list_2 = torch.LongTensor([(b.GetBeginAtomIdx(), b.GetEndAtomIdx()) for b in mol_graph_2.GetBonds()])
    # adjacency_matrix = torch.zeros(num_nodes2, num_nodes2)
    ...
    # # 根据阈值删除不重要的节点
    # nodes_to_remove = [node for node, importance in enumerate(node_importance) if importance < threshold]
    # x2 = [x for x in x2 if x not in nodes_to_remove]
...
# df_all_pos_ddi = pd.read_csv('data/twosides.csv')
...
class DrugDataset(Dataset):
    def __init__(self, tri_list, ratio=1.0, neg_ent=1, disjoint_split=True, shuffle=True, data_save=False):
        ...
        if data_save:
            for h, t, *_ in self.tri_list:
                positive_samples_data.append([h, t])
            with open('data/shullfdrugdata5.csv', 'a', newline='') as csvfile:
                csv_writer = csv.writer(csvfile)
                # 检查文件是否为空，如果不为空则不写入标题行
                if csvfile.tell() == 0:
                    csv_writer.writerow(['head', 'tail'])
